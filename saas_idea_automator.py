...
async def process_single_idea(idea_name, idea_description, idea_embedding, processed_ideas_set, session, semaphore, batch_stats):
    """Async function to research, extract facts, rate, save state, and potentially queue re-generation for a single idea."""
    async with semaphore:
        ...
        try:
                ...
                # --- Focused Re-generation Logic ---
                    ...
                    if scores and justifications:
                        ...
                        if regen_response:
                             ...
                             # Add suggested ideas to the pending queue instead of just logging
            ...
            # Update state even if re-generation was triggered
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        while run_count < config.MAX_RUNS:
            ...
            if run_count > 1:
                # ... (adaptive logic remains the same) ...
            ...
            # --- Idea Generation: Prioritize Pending, then Multi-Step, Variation, or New ---
            initial_ideas = []
            generation_mode = "New Ideas (Default)" # Default mode

            # Check for pending re-generated ideas first
            pending_ideas = state_manager.fetch_and_clear_pending_ideas(limit=config.IDEAS_PER_BATCH)
            if pending_ideas:
                initial_ideas = pending_ideas
                generation_mode = "Pending Re-generated"
                logging.info(f"Processing {len(initial_ideas)} pending re-generated ideas from queue.")

            # If no pending ideas, proceed with normal generation logic
            ...
            # ... (semantic filter logic remains the same) ...
            ...
            # --- Self-Critique Step ---
            # ... (self-critique logic remains the same) ...
            ideas = []
            ...
            # ... (embedding generation for final ideas remains the same) ...
