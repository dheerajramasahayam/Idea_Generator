...
def get_promising_themes(top_n=5):
    ...
    high_rated_ideas_data = state_manager.get_high_rated_ideas(limit=50) # Get up to 50 high-rated ideas
...
# These remain the same as in the previous version, using api_clients and config
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        while run_count < config.MAX_RUNS:
            ...
            # Run analysis every few runs (e.g., every 5 runs) after the first one
            if run_count > 1 and (run_count - 1) % 5 == 0:
                promising_themes = get_promising_themes(top_n=5) # Get top 5 keywords
            ...
            if not ideas:
                ...
                await asyncio.sleep(10) # Wait a bit longer
                continue

            logging.info(f"Attempting to process {len(ideas)} filtered ideas.")
            tasks = []
            new_ideas_to_process = 0
            for idea in ideas:
                 if idea.lower() not in processed_ideas_set:
                     ...
                     tasks.append(task)
                     new_ideas_to_process += 1
                 else:
                     logging.info(f"Skipping idea generated by AI as it was already processed: '{idea}'")
            ...
            logging.info(f"===== Finished Run {run_count}/{config.MAX_RUNS} =====")

            # --- Uptime Kuma Ping ---
            await api_clients.ping_uptime_kuma(session)
            # -----------------------
